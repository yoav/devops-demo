...
if node.my_env == 'production'
 propFilter	= "deploy.env+=production"
elsif node.my_env == 'staging'
 propFilter	= "deploy.env+=staging"
end


ruby_block "deploy latest version: #{my_env}" do
 block do
name =  "frocky-#{my_env}.war"
fname = "workspace-1.0-SNAPSHOT.war"
base_url   = "http://#{node['art_host']}:#{node['art_port']}/artifactory/cloud-deploy-local/frocky/workspace/1.0-SNAPSHOT"
search_url = "http://#{node['art_host']}:#{node['art_port']}/artifactory/cloud-deploy-local/frocky/workspace/1.0-SNAPSHOT/#{fname};#{propFilter}"
...
FileUtils.rm_rf("#{node[:tomcat7][:base]}/webapps/frocky-#{my_env}")
